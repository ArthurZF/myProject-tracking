    ...
    <transition name="fade">
      <el-form label-width="80px" :model="form" class="message-form" v-show="show" :rules="loginRules" status-icon
        ...
        <el-form-item label="账号" prop="account" ref="account">
          <el-input v-model.trim="form.account" placeholder="请输入账号" auto-complete="off" clearable>
          </el-input>
        </el-form-item>
        ...
        <el-form-item>
          <el-checkbox v-model.trim="form.savePassword" class="float-left">
            记住密码
          </el-checkbox>
          <p class="float-right forget-password pointer">忘记密码？</p>
        </el-form-item>
        <el-form-item>
          <el-button type="primary" class="float-left ele-button" @click="login" :loading="isLoading">
            登陆
          </el-button>
          <el-button type="primary" plain class="float-right ele-button" @click="registered">
            注册
          </el-button>
        </el-form-item>
      </el-form>
      ...
      async loginPost() {
        ...
        if (res.resData.accountId == "") {
          ...
          setTimeout(() => {
            ...
            window.location.replace("./workorder.html");
          }, 1000);
        } else {
          this.$refs.login.clearValidate();
          this.$message({
